...
from nasdaq_data_link_mcp_os.resources.equities_360.statistics import get_company_stats
from nasdaq_data_link_mcp_os.resources.world_data_bank.indicators import get_indicator_value
...
class TestWorldBankTools:
    ...
    def test_get_indicator_value_basic(self):
        ...
        try:
            ...
            assert "api" in str(e).lower() or "key" in str(e).lower() or "unauthorized" in str(e).lower()
...
class TestToolRegistration:
    ...
    def test_tool_functions_exist(self):
        ...
        for tool in tools:
            ...
            assert hasattr(tool, '__name__'), f"Tool missing name attribute"
            assert hasattr(tool, '__doc__'), f"Tool missing docstring"
            assert len(tool.__name__) > 0, f"Tool has empty name"
